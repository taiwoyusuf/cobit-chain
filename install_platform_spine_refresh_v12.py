...
MARKER = "COBITCHAIN_PLATFORM_SPINE_REFRESH_V12_VERIFICATION_ACTIVE"
...
if not route_exists(text, "/platform"):
    block_parts.append(r'''
@app.route("/platform")
def cobitchain_platform_command_center_v12():
    from pathlib import Path
...
def cobitchain_platform_route_registry_v12():
    from pathlib import Path
    html_path = Path(__file__).with_name("platform_route_registry_command_center.html")
    return html_path.read_text(encoding="utf-8")
''')

if not route_exists(text, "/api/platform/spine-refresh/v12/demo"):
    block_parts.append(r'''
...
Path("platform_spine_refresh_v12_urls.txt").write_text(
    "\n".join([
        ...
        "http://127.0.0.1:5000/api/platform/spine-refresh/v12/demo"
    ]),
    encoding="utf-8"
)

print("COBIT-Chain Platform Spine Refresh v12 installed.")
print("Updated files:")
print("  platform_ab_command_center.html")
print("  platform_route_registry_command_center.html")
print("  platform_spine_refresh_v12_urls.txt")
print("Verification marker:")
print("  COBITCHAIN_PLATFORM_SPINE_REFRESH_V12_VERIFICATION_ACTIVE")
