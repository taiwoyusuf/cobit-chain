...
MARKER = "COBITCHAIN_PLATFORM_DEPLOYMENT_RELEASE_COMMAND_CENTER_V1_ACTIVE"
...
if not routes_to_add:
    raise SystemExit("All desired Deployment Release routes already exist. No changes made.")
...
Path("platform_deployment_release_installed_urls.txt").write_text(
    "\n".join([f"http://127.0.0.1:5000{route}" for route in routes_to_add]),
    encoding="utf-8"
)

print("COBIT-Chain Platform Deployment and Release Command Center installed.")
