...
MARKER = "COBITCHAIN_PLATFORM_MCP_SERVER_COMMAND_CENTER_V1_ACTIVE"
...
if not routes_to_add:
    raise SystemExit("All desired MCP server routes already exist. No changes made.")
...
Path("platform_mcp_server_installed_urls.txt").write_text(
    "\n".join([f"http://127.0.0.1:5000{route}" for route in routes_to_add]),
    encoding="utf-8"
)

print("COBIT-Chain MCP Server Command Center installed.")
