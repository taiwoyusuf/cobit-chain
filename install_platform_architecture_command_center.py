...
MARKER = "COBITCHAIN_PLATFORM_ARCHITECTURE_COMMAND_CENTER_V1_ACTIVE"
...
if not routes_to_add:
    raise SystemExit("All desired architecture routes already exist. No changes made.")
...
Path("platform_architecture_installed_urls.txt").write_text(
    "\n".join([f"http://127.0.0.1:5000{route}" for route in routes_to_add]),
    encoding="utf-8"
)

print("COBIT-Chain Platform Architecture Command Center installed.")
