...
MARKER = "COBITCHAIN_PLATFORM_GOVERNANCE_VISION_COMMAND_CENTER_V1_ACTIVE"
...
desired_routes = [
    "/platform/governance-vision",
    "/platform/vision",
    "/governance-vision",
    "/cobitchain-governance-vision"
]
...
if not routes_to_add:
    raise SystemExit("All desired Governance Vision routes already exist. No changes made.")
...
Path("platform_governance_vision_installed_urls.txt").write_text(
    "\n".join([f"http://127.0.0.1:5000{route}" for route in routes_to_add]),
    encoding="utf-8"
)

print("COBIT-Chain Governance Vision Command Center installed.")
